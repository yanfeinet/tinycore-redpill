...
rpt_download_remote()
{
    ...
    URL="${1}"
    filename="${URL##*/}"
    len_start=`expr ${#filename} - 4`
    if test ${filename:${len_start}:4} = ".tgz" -a  ${URL:0:12} = "https://gith"
    then
        pr_info "Redirecting..."
    elif test ${filename:${len_start}:4} = "6_64" -a  ${URL:0:12} = "https://gith"
    then
        pr_info "Redirecting..."
    else
        URL=${URL/"https://raw.githubusercontent.com/"/${acc_url}"https://raw.githubusercontent.com/"}
        URL=${URL/"https://github.com"/${acc_url}"https://raw.githubusercontent.com"}
        URL=${URL/"/raw/"/"/"}
        pr_info "Redirecting..."
    fi
